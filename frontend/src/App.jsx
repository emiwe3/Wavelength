...
export default function App() {
  ...
  const fetchStatus = async (phoneOverride) => {
    try {
      const { data } = await axios.get(`${API}/api/status`, { withCredentials: true });
      const knownPhone = phoneOverride || localStorage.getItem("wl_phone");
      if (data.phone && data.phone === knownPhone) {
        setStatus(data);
      } else if (knownPhone) {
              ...
              <>
                ...
                {needsSlack && (
                  <section className="card">
                    ...
                    <ConnectButton service="Slack" connected={false} onClick={() => {
                      const savedPhone = localStorage.getItem("wl_phone") || status.phone;
