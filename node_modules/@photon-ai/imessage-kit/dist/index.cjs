...
var nodeTypedstream = require('node-typedstream');
...
var IMessageDatabase = class {
  ...
  extractTextFromAttributedBody(attributedBody) {
    ...
    try {
      ...
      const decoded = nodeTypedstream.Unarchiver.open(buffer, nodeTypedstream.Unarchiver.BinaryDecoding.decodable).decodeAll();
      if (!decoded) return null;
      const items = Array.isArray(decoded) ? decoded : [decoded];
      for (const item of items) {
        if (item instanceof nodeTypedstream.NSAttributedString && item.string) {
          return item.string;
        }
        if (item?.values && Array.isArray(item.values)) {
          for (const val of item.values) {
            if (val instanceof nodeTypedstream.NSAttributedString && val.string) {
              return val.string;
            }
          }
        }
  ...
  mapReactionType(type) {
    ...
    const isInAddRange = typeNum >= 2e3 && typeNum <= 2005;
    const isInRemoveRange = typeNum >= 3e3 && typeNum <= 3005;
    if (!isInAddRange && !isInRemoveRange) {
      return { isReaction: false, reactionType: null, isReactionRemoval: false };
    }
    const isRemoval = isInRemoveRange;
    const baseType = isRemoval ? typeNum - 1e3 : typeNum;
    const typeMap = {
      2e3: "love",
      2001: "like",
      2002: "dislike",
...
function buildTempFilenameTemplate(filePath) {
...
var generateSecureFilename = (ext) => {
  return `imsg_temp_${crypto.randomBytes(8).toString("hex")}${ext}`;
};
var convertImageToJPEG = async (inputPath, outputPath) => {
  const output = outputPath || path.join(TEMP_DIR2, generateSecureFilename(".jpg"));
  try {
    await execFileAsync("sips", ["-s", "format", "jpeg", inputPath, "--out", output], { timeout: 1e4 });
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (contentType.includes("avif") || contentType.includes("webp")) {
        ...
        const tempPath = path.join(TEMP_DIR2, generateSecureFilename(ext2));
      ...
      const path2 = path.join(TEMP_DIR2, generateSecureFilename(ext));
      fs.writeFileSync(path2, buffer, { flag: "wx", mode: 384 });
      return path2;
    } catch (error) {
...
var convertToCompatibleFormat = async (filePath) => {
  ...
  const output = isOurTemp ? path.join(TEMP_DIR2, fileName) : path.join(TEMP_DIR2, generateSecureFilename(".jpg"));
...
var MessageScheduler = class {
  ...
  destroyed = false;
  ticking = false;
  constructor(sdk, config, events) {
    this.sdk = sdk;
    this.config = {
      checkInterval: config?.checkInterval ?? 1e3,
      debug: config?.debug ?? false
    };
    this.events = events ?? {};
    this.start();
      ...
      for (const msg of data.recurring) {
        ...
        if (msg.status === "pending" && nextSendAt > now) {
          this.recurring.set(msg.id, {
            ...
            createdAt: new Date(msg.createdAt),
            nextSendAt,
            endAt: msg.endAt ? new Date(msg.endAt) : void 0
          });
          imported++;
        } else {
          skipped++;
   ...
   */
  async sendRecurringMessage(msg) {
    try {
      if (this.config.debug) {
...
function parseAtExpression(expression) {
  const now = /* @__PURE__ */ new Date();
...
var Reminders = class {
  ...
  constructor(sdk, events) {
    this.scheduler = new MessageScheduler(
      sdk,
      { checkInterval: 1e3, debug: false },
      {
        onSent: (msg, result) => {
  ...
  get(id) {
    ...
    if (!sched || sched.status !== "pending") return void 0;
    return reminder;
  }
  /**
   ...
   */
  list() {
    return Array.from(this.reminders.values()).filter((r) => this.scheduler.get(r.id)?.status === "pending").sort((a, b) => a.scheduledFor.getTime() - b.scheduledFor.getTime());
  }
  /**
