...
import { existsSync, readdirSync, lstatSync, unlinkSync, writeFileSync } from 'fs';
...
var convertImageToJPEG = async (inputPath, outputPath) => {
  const output = outputPath || join(TEMP_DIR2, generateSecureFilename(".jpg"));
  try {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (contentType.includes("avif") || contentType.includes("webp")) {
        ...
        const tempPath = join(TEMP_DIR2, generateSecureFilename(ext2));
      ...
      const path2 = join(TEMP_DIR2, generateSecureFilename(ext));
      writeFileSync(path2, buffer, { flag: "wx", mode: 384 });
      return path2;
    } catch (error) {
...
var convertToCompatibleFormat = async (filePath) => {
  ...
  const output = isOurTemp ? join(TEMP_DIR2, fileName) : join(TEMP_DIR2, generateSecureFilename(".jpg"));
